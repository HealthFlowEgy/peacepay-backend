    ...
    <div class="row mt-20 mb-20-none">
        <div class="col-xl-12 col-lg-12 mb-20">
            <div class="custom-card mt-10">
                ...
                <div class="card-body">
                    <form action="{{ setRoute('user.policies.update', $policy->id) }}" class="card-form policy-form" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                    
                        @php
                            $policyFields = json_decode($policy->fields, true) ?? [];
                        @endphp
                    
                        <div class="row">
                            <div class="col-xl-6 col-lg-6 form-group">
                                <label>Name:</label>
                                <input type="text" name="name" class="form--control" placeholder="Name" value="{{ old('name', $policy->name) }}">
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-xs-12 col-sm-12 col-md-12">
                                <div class="form-group">
                                    ...
                                    <textarea class="form--control" style="height:150px" name="description" placeholder="Description">{{ old('description', $policy->description) }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6 form-group">
                                ...
                                <select name="fields[delivery_fee_payer]" class="form--control">
                                    <option value="buyer" {{ old('fields.delivery_fee_payer', $policyFields['delivery_fee_payer'] ?? '') == 'buyer' ? 'selected' : '' }}>Buyer</option>
                                    <option value="seller" {{ old('fields.delivery_fee_payer', $policyFields['delivery_fee_payer'] ?? '') == 'seller' ? 'selected' : '' }}>Seller</option>
                                </select>
                            </div>
                            ...
                            <div class="col-md-6 form-group">
                                ...
                                <select name="fields[return_fee_payer]" class="form--control">
                                    <option value="buyer" {{ old('fields.return_fee_payer', $policyFields['return_fee_payer'] ?? '') == 'buyer' ? 'selected' : '' }}>Buyer</option>
                                    <option value="seller" {{ old('fields.return_fee_payer', $policyFields['return_fee_payer'] ?? '') == 'seller' ? 'selected' : '' }}>Seller</option>
                                </select>
                            </div>
                            ...
                            <!-- <div class="col-md-6 form-group">
                                ...
                                <div class="radio-wrapper">
                                    <div class="radio-item">
                                        <input type="radio" name="fields[has_delivery_timeframe]" id="timeframe_yes" value="1" {{ old('fields.has_delivery_timeframe', $policyFields['has_delivery_timeframe'] ?? '') == '1' ? 'checked' : '' }}>
                                        <label for="timeframe_yes">
                                            Yes, include delivery timeframe
                                        </label>
                                    </div>
                                    <div class="radio-item">
                                        <input type="radio" name="fields[has_delivery_timeframe]" id="timeframe_no" value="0" {{ old('fields.has_delivery_timeframe', $policyFields['has_delivery_timeframe'] ?? '') == '0' ? 'checked' : '' }}>
                                        <label for="timeframe_no">
