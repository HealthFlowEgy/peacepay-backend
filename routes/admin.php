...
use App\Http\Controllers\Admin\EscrowCategoryController;
...
Route::prefix('admin')->name('admin.')->group(function () {
    ...
    // Escrow category routes
    Route::controller(EscrowCategoryController::class)->prefix('escrow-category')->name('escrow.category.')->group(function () {
        Route::get('index', 'index')->name('index');
        Route::post('store', 'store')->name('store');
        Route::put('status/update', 'statusUpdate')->name('status.update');
        Route::put('update', 'update')->name('update');
    ...
    // Escrow routes
    Route::controller(EscrowController::class)->prefix('escrow')->name('escrow.')->group(function () {
        Route::get('index', 'index')->name('index'); 
        Route::get('ongoing', 'ongoing')->name('ongoing'); 
        Route::get('payment-pending', 'paymentPending')->name('paymentPending'); 
        Route::get('approval-pending', 'approvalPending')->name('approvalPending'); 
        Route::get('released', 'released')->name('released'); 
        Route::get('active-dispute', 'activeDispute')->name('activeDispute'); 
        Route::get('disputed', 'disputed')->name('disputed'); 
        Route::get('canceled', 'canceled')->name('canceled'); 
        Route::get('refunded', 'refunded')->name('refunded'); 
        //escrow details
        Route::get('details/{id}', 'escrowDetails')->name('details'); 
        //escrow chate 
        Route::get('conversation/{id}', 'escrowChat')->name('chat'); 
        ...
        Route::put('manual-payment-rejected', 'manualPaymentRejected')->name('manual.payment.rejected');
    });
    ...
    Route::controller(BlogController::class)->prefix('blog')->name('blog.')->group(function () {
         //Blogs routes
         Route::get('/','blogView')->name('index');
         Route::post('store', 'blogItemStore')->name('store');
         Route::put('status/update','blogStatusUpdate')->name('status.update');
         Route::get('edit/{id}','blogEdit')->name('edit');
         Route::put('update', 'blogItemUpdate')->name('update');
         Route::delete('delete','blogItemDelete')->name('delete');
    ...
    Route::controller(SupportTicketController::class)->prefix('support-ticket')->name('support.ticket.')->group(function () {
        ...
        Route::post('message/reply','messageReply')->name('message.reply');
