...
trait SslcommerzTrait
{
    public function sslcommerzInit($output = null) {
        ...
        $currency = $output['amount']->gateway_cur_code??"BDT";
...
\Log::info('error mail : ,'. $e->getMessage());
    ...
    public function sslcommerzInitApi($output = null) {
        ...
        $reference = generateTransactionReference();
        $amount = $output['amount']->total_payable_amount ? number_format($output['amount']->total_payable_amount,2,'.','') : 0;
        $currency = $output['gateway_currency']['currency_code']??"BDT";
        ...
        $post_data['currency'] = $currency;
        $post_data['tran_id'] =  $reference;
        $post_data['success_url'] =  route('api.v1.add-money.ssl.success',"r-source=".PaymentGatewayConst::APP);
        $post_data['fail_url'] = route('api.v1.add-money.ssl.fail',"r-source=".PaymentGatewayConst::APP);
        $post_data['cancel_url'] = route('api.v1.add-money.ssl.cancel',"r-source=".PaymentGatewayConst::APP);
        # $post_data['multi_card_name'] = "mastercard,visacard,amexcard";  # DISABLE TO DISPLAY ALL AVAILABLE
