...
class PaymentGatewaysController extends Controller
{
    ...
    public function registerSlugTypes($type) {
        $slug_types = [
            'view' => [
                ...
                'money-out' => [
                    'automatic' => 'automaticMoneyOutView',
            ...
            'edit'  => [
                ...
                'money-out'     => [
                    'automatic' => 'automaticMoneyOutEdit',
            ...
            'update'    => [
                ...
                'money-out'     => [
                    'automatic' => 'automaticMoneyOutUpdate',
            ...
            'store'     => [
                ...
                'money-out'     => [
                    'automatic' => 'automaticMoneyOutStore',
    ...
    /**
     * Display Money Out Automatic Gateways
     * @return view
     */
    public function automaticMoneyOutView() {
        $page_title = __("Automatic Withdraw Gateway");
    ...
    /**
     * Function for edit new manual payment gateway
     * @param \Illuminate\Http\Request $request
     * @return view
     */
    public function automaticMoneyOutEdit($alias) {
        $page_title = __("Automatic Withdraw Edit");
        $gateway = PaymentGateway::moneyOut()->automatic()->gateway($alias)->firstOrFail();
        return view('admin.sections.payment-gateways.money-out.automatic.edit',compact(
            'page_title',
            'gateway',
        ));
    }
     ...
     * @return view
     */
    public function automaticMoneyOutStore(Request $request) {
        $gateway_name = $request->gateway_name;
        $validator = Validator::make($request->all(),[
            'gateway_name'              => ['required','string','max:60',Rule::unique('payment_gateways','alias')->where(function($query) use ($gateway_name) {
                $alias = Str::slug($gateway_name);
                $query->where('slug',PaymentGatewayConst::money_out_slug())->where('type',PaymentGatewayConst::AUTOMATIC)->where('alias',$alias);
            })],
            'gateway_title'             => 'required|string|max:60',
            'supported_currencies'      => 'required|array',
            'supported_currencies.*'    => 'string|max:30',
        ...
        $validator->after(function ($validator) use ($gateway_name) {
            // Search Gateway is unique or not
            if(PaymentGateway::moneyOut()->automatic()->gateway(Str::slug($gateway_name))->exists()) {
                $validator->errors()->add(
                    'gateway_name', 'The gateway name has already been taken.'
                );
            }
        ...
        if($validator->fails()) {
            return back()->withErrors($validator)->withInput()->with('modal','automatic-money-out');
        }
        $validated = $validator->validate();
        ...
        $validated['slug']          = Str::slug("Money Out");
        $validated['type']          = "AUTOMATIC";
        $validated['name']          = $validated['gateway_name'];
        $validated['title']         = $validated['gateway_title'];
        $validated['alias']         = Str::slug($validated['gateway_name']).'-'.Str::slug("Money Out");
        $validated['last_edit_by']  = Auth::user()->id;
   ...
   /**
     * Function for update new manual payment gateway automatic
     * @param \Illuminate\Http\Request $request
     * @return view
     */
    public function automaticMoneyOutUpdate(Request $request,$alias) {


        $validator_gateway = Validator::make(['alias' => $alias, 'mode' => $request->mode],[
            'alias'     => 'exists:payment_gateways',
            'mode'      => "required|string|in:".PaymentGatewayConst::ENV_SANDBOX.",".PaymentGatewayConst::ENV_PRODUCTION,
        ],[
        ...
        foreach($validated[$input_field_base_name] ?? [] as $currency => $item) {
            ...
            $item['currency_code']      = $currency;
            $item['name']               = $gateway->name . " " . $currency;
            $item['alias']              = Str::slug($gateway->name . " " . $currency ." ".$gateway->slug. " " . $gateway->type);
            $item['payment_gateway_id'] = $gateway->id;
            $item['rate']               = $item['rate'] ?? 1;
            $item['image']              = null;
