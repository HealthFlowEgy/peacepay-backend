...
class ForgotPasswordController extends Controller
{
    ...
    public function sendCode(Request $request)
    {
        ...
        return redirect()->route('user.password.forgot.code.verify.form',$token)->with(['success' => [__('Varification code sended to your email address')]]);
    }
    ...
    public function sendCodeMobile(Request $request)
    {
        ...
        session()->put('mobile',$request->credentials);
        try{
            $update_data = [
        ...
        return redirect()->route('user.password.forgot.code.verify.mobile')->with(['success' => [__('Varification code sended to your mobile')]]);
    }


    public function showVerifyFormMobile() {
        $page_title = setPageTitle("Verify User");
        
        $user = User::where('mobile',session()->get('mobile'))->first();

        if(!$user) return redirect()->back()->with(['error' => [__('Password Reset Token Expired')]]);
        ...
        return view('user.auth.forgot-password.verify-mobile',compact('page_title','full_mobile','resend_time'));
    }
    ...
    public function resendCode($token)
    {
        ...
        return redirect()->route('user.password.forgot.code.verify.form',$token)->with(['success' => [__('Varification code resend success!')]]);
